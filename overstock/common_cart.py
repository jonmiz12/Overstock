...
class CommonCart(BasePage):
    ...
    def increase_item_quantity(self, item_els: Locator, item_quantity: Locator, item_error: Locator,
                               increase_quantity: Locator, cart_button: Locator, actual_item: Locator, index: int,
                               iterator: int, remove_counter: int, cart: list[dict[str, str | int | float]], amount: int) -> list[dict]:
        cart = self.validate_maximum_quantities(item_els, item_quantity, item_error, increase_quantity,
                                                cart_button, actual_item, index, cart, amount)
    ...
    def decrease_item_quantity(self, item_els: Locator, item_quantity: Locator, item_error: Locator,
                               decrease_quantity: Locator, cart_button: Locator, actual_item: Locator, index: int,
                               iterator: int, remove_counter: int, cart: list[dict[str, str | int | float]], amount: int) -> (list[dict], int):
    ...
    @staticmethod
    def change_cart_quantities(self, cart: list[dict[str, str | int | float]], amounts: [int], item_els: Locator, item_name: Locator,
                               increase_quantity: Locator, decrease_quantity: Locator, item_quantity: Locator,
                               item_error: Locator, cart_button: Locator) -> list[dict[str, str | int | float]]:
        remove_counter = 0
            ...
            for actual_item in item_els.all():
                ...
                if amount > 0:
                    cart = self.increase_item_quantity(self, item_els, item_quantity, item_error,
                                                       increase_quantity, cart_button, actual_item, index,
                    ...
                    cart, remove_counter = self.decrease_item_quantity(self, item_els, item_quantity, item_error,
                                                                       decrease_quantity, cart_button, actual_item, index,
                                                                       iterator, remove_counter, cart, amount)
                break
        return cart
    ...
    def validate_maximum_quantities(self, item_els: Locator, item_quantity: Locator, item_error: Locator,
                                    item_adjust: Locator, cart_button: Locator, actual_item_el: Locator, item_el_index,
                                    cart: list[dict[str, str | int | float]], amount: int = None) -> list[dict[str, str | int | float]]:
    ...
    def validate_cart(self, cart: list[dict[str, str | int | float]], item_els: Locator, item_name: Locator, item_vendor: Locator,
        ...
        for item in item_els.all():
            ...
            actual_item_quantity = float(item.locator(item_quantity).get_attribute("value"))
            ...
            expected_price = round(float(item["raw_price"]), 2)
            ...
            if len(item_failed_assertions) > 0:
    ...
    def assert_quantity(self, initial_amount: int, amount: int, item_el: Locator, item_quantity: Locator,
                        item: dict[str, str | int | float]) -> dict[str, str | int | float]:
