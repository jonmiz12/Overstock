...
class BasePage:
    ...
    def create_item(self, cart: list, item_data: dict[str, str | int | float], total_price: int = 0, quantity: int = 0,
                    max_quantity: float = None) -> list:
        item = {
            'name': item_data["name"],
    ...
    @staticmethod
    def update_quantity(cart: list, quantity: int, item_name: str):
        for item in cart:
    ...
    def custom_click(self, element: Locator):
    ...
    @staticmethod
    def apply_discount_to_item(item: dict[str, str | int | float]):
        global DISCOUNT
    ...
    # Returns the current quantities of all items based on their locators and the item_quantity element.
    @staticmethod
    ...
    def adjust_cart_to_quantities(self, cart: list[dict[str, str | int | float]], item_els: Locator, item_quantity: Locator) -> list[dict[str, str | int | float]]:
    ...
    @staticmethod
    def calculate_amount_by_max_quantity(actual_amount: int, item: dict[str, str | int | float], amount: int) -> int:
    ...
    def add_failed_assertion(cls, error, method_name):
        ...
        if isinstance(cls.failed_assertions, list):
            cls.failed_assertions.append({"methodName": method_name, "body": error, "timeStamp": current_datetime})
        else:
    ...
    #     assert len(cls.failed_assertions) == 0    # Prints all failed assertions in red text. If any failed assertions exist, it will raise an assertion error.

    @classmethod
    def print_failed_assertions(cls) -> []:
        for failed_assertion in cls.failed_assertions:
            allure.attach(failed_assertion["timeStamp"] + "\n" + failed_assertion["body"], name=failed_assertion["methodName"], attachment_type=allure.attachment_type.TEXT)
            # assert len(cls.failed_assertions) == 0, f"Product errors found: {cls.failed_assertions}"
